 ...
 *
 * unit task:
 * Execute OpenVeo Devices API unit tests.
 *
 * unit environment variables:
 * - TRAVIS_BRANCH: The name of the openveo-devices-api branch to test (default: develop)
...
var projectDirectory = path.join(workingDirectory, 'node_modules/@openveo/devices-api');
var homeDirectory = openVeoApi.fileSystem.getConfDir();
var devicesApiBranch = process.env.TRAVIS_BRANCH || 'develop';
...
plan.local(['unit'], function(local) {
  ...
  try {
    ...
    local.exec('git clone --branch=' +
               devicesApiBranch +
               ' --single-branch https://github.com/veo-labs/openveo-devices-api.git ' +
               projectDirectory);

    // Install openveo-devices-api dependencies and compile sources
